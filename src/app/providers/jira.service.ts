...
import {IssueSearchResultModel} from "../../../../hotshot-logger/src/app/domain-models/issue-search-result.model";
import {promise} from "selenium-webdriver";
import {WorkLogModel} from "../../../../hotshot-logger/src/app/domain-models/work-log.model";
import {FilterModel} from "../../../../hotshot-logger/src/app/domain-models/filter.model";
...
export class JiraService {
  ...
  private post(path, body) {
    ...
    return this.http.post(`https://${domain}/rest/api/2/${path}`, body, options);
  }
  ...
  getCurrentEmployeeHours(date: Date): Observable<Array<any>> {
    return this.getIssuesByJql(`worklogDate = ${moment(date).format('YYYY-MM-DD')} and worklogAuthor = currentUser() &fields=['']`)
      .pipe(
        map((result: { issues: Array<{ key: string }> }) => result.issues.map(issue => issue.key)),
      ).pipe(
        mergeMap(
          (keys: Array<string>) => {
            return combineLatest(keys.map(issueKey => {
              return this.getIssueWorkLog(issueKey).pipe(
  ...
  logWork(ticket: string, workLogModel: WorkLogModel): Observable<any> {
    return this.post(`${ticket}/worklog?adjustEstimate=auto`, workLogModel)
  }
